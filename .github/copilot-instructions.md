...
- `src/App.tsx` wires the route graph through `createBrowserRouter`, wraps everything in `AuthProvider` + `GalleryProvider`, and renders the ambient `AppBackdrop` + `GalleryModalRenderer` so every page can trigger the modal.
