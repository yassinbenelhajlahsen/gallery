...
- **Provider order matters:** `AuthProvider` → `GalleryProvider` → `ToastProvider` → `AppBackdrop` → `RouterProvider` + `GalleryModalRenderer`. Inner providers consume outer ones. Never mount providers inside a page.
