...
```
AuthProvider → ToastProvider → AppBackdrop → RouterProvider
                                                    └─ RequireAuth
...
Optional branding overrides in `src/config.ts`: `VITE_BRAND_DISPLAY`, `VITE_SITE_TITLE`, `VITE_SITE_DESCRIPTION`, `VITE_LOGIN_HEADING`, `VITE_LOGOUT_TOAST`, `VITE_NOT_FOUND_TEXT`
